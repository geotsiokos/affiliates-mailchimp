...
class Mailchimp_Api {
	...
	private function request_url( $type, $params = array(), $list_params = array() ) {
		switch ( $type ) {
			case 'get' :
				$result = self::LISTS . '?' . http_build_query( $parameters );
				break;
			case 'update' :
			case 'check' :
				$result = self::LISTS . '/' . $list_params['list_id'] . self::MEMBERS . '/' . md5( $parameters['email_address'] );
				break;
			case 'new' :
	...
	public function add_interest_category( $list_parameters = array(), $parameters = array() ) {
		...
		if ( isset( $list_parameters['list_id'] ) ) {
			if ( isset( $parameters['title'] ) && isset( $parameters['type'] ) ) {
				$interest_category_url = $this->request_url( 'interest_category', $parameters, $list_parameters );
	...
	public function add_interest( $list_parameters = array(), $parameters = array() ) {
		...
		if ( isset( $list_parameters['list_id'] ) ) {
			if ( isset( $list_parameters['interest_category_id'] ) && isset( $parameters['name'] ) ) {
				$interest_url = $this->request_url( 'interest', $parameters, $list_parameters );
				if ( $interest_url ) {
