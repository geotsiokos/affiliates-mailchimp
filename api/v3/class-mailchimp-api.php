...
class Mailchimp_Api {
	...
	/**
	 * Check if given email is a member in the list
	 *
	 * @param $list_id
	 * @param $email
	 * @return array|mixed|null|object
	...
	public function subscribe( $list_id, $email, $subscribed = true, $merge_fields = array(), $list_interests = array() ) {
		...
		error_log( 'api.subscribe, Subscribing ' . $email );
	...
	{
		$url = "https://{$this->data_center}.api.mailchimp.com/{$this->api_version}/";
		if (!empty($extra)) {
			$url .= $extra;
		}
		if (!empty($params)) {
	...
	protected function applyCurlOptions( $method, $url, $params = array(), $headers = array() ) {
		...
		return array(
			...
			CURLOPT_HTTPHEADER => array_merge(
				array(
					'content-type: application/json',
					"user-agent: Affiliates MailChimp/{$env->version}; PHP/{$env->php_version}; WordPress/{$env->wp_version};",
				), $headers )
			);
	}
	/**
	...
	protected function processCurlResponse( $curl ) {
		$response = curl_exec( $curl ); error_log( 'response' ); error_log( $response );
		$err = curl_error( $curl );
		...
		if ( $info['http_code'] >= 400 && $info['http_code'] <= 500 ) {
			...
			error_log( $data['title'] . ' :: ' . $data['detail'] . ' , ' . $data['status'] );
		}
		if ( $info['http_code'] >= 500 ) {
			//throw new MailChimp_Server_Error( $data['detail'], $data['status'] );
			error_log( $data['title'] . ' :: ' . $data['detail'] . ' , ' . $data['status'] );
		}
		return null;
	}
