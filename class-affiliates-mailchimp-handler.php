...
class Affiliates_Mailchimp_Handler {
	 ...
	 */
	private static function check_requests() {
		$options = get_option( 'affiliates-mailchimp' );
		...
		if ( $options['api_key'] ) {
			...
			//lists/650653d2af/interest-categories/b74d5aea02/interests/22f089e85d
			if ( !isset( $interest_id ) ) {
				$interest_params = array(
						'name' => $interest
				);
			...
			if ( isset( $check['status'] ) ) {
				if ( $check['status'] == ( 'unsubscribed' || 'subscribed' ) ) {
					// patch
					$api->update_subscriber( $list_parameters, $user_data );
				}
			} else {
				// check the opt-in option
				if ( $need_confirm == '1' ) {
				...
				// add new subscriber
				$api->new_subscriber( $list_parameters, $user_data );
			}
		}
	}
